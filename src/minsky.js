...
export class RegisterMachine {
    constructor(nodes, registers) {
        this.nodes = nodes || {};
        this.registers = registers || {};
        // each node's transition is assigned a label, and in this loop
        // this label is converted into a node
        for (let n in this.nodes) {
            ...
            // If a node is Plus Node, it will have only one
            // transition state
            if (node.on_increment !== halt_node) {
                if (!this.nodes.hasOwnProperty(node.on_increment)) {
                    throw new Error(`register machine does not have node named: ${node.on_increment}`);
                }
            ...
            if (node.on_decrement !== halt_node) {
                if (!this.nodes.hasOwnProperty(node.on_decrement)) {
                    throw new Error(`register machine does not have node named: ${node.on_decrement}`);
                }
                node.on_decrement = this.nodes[node.on_decrement];
            }

            if (node.on_empty !== halt_node) {
                if (!this.nodes.hasOwnProperty(node.on_empty)) {
                    throw new Error(`register machine does not have node named: ${node.on_empty}`);
                }
